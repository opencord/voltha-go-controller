...
func printVersion() {
	fmt.Println("VGC Version")
	fmt.Println(vgcInfo.Version)
}

func stop(ctx context.Context, kvClient kvstore.Client, ofa *ofagent.OFAgent) {
...
func main() {
	...
	//flag.Parse()
	//config.KVStoreEndPoint = config.KVStoreHost + ":" + strconv.Itoa(config.KVStorePort)
	...
	ofa, err1 := ofagent.NewOFAgent(&ofagent.OFAgent{
...
// GetStringInfraLogLevel to convert infra log level to string
func GetStringInfraLogLevel(logLevel log.LogLevel) string {
	switch logLevel {
	case log.FatalLevel:
		return "critical"
	case log.ErrorLevel:
