...
func (mh *MetersHandle) GetMeter(cntx context.Context, meterID string, w http.ResponseWriter, r *http.Request) {
	logger.Infow(cntx, "Received Get Meter request", log.Fields{"metreId": meterID})
...
func (mh *MetersHandle) GetAllMeters(cntx context.Context, w http.ResponseWriter, r *http.Request) {
	logger.Info(ctx, "Request for Get All Meter Info from device")
	...
	logger.Infow(ctx, "Fetching all Meter Info from device", log.Fields{"MetersList": metersList})
}
