...
func (pfh *PendingFlowHandle) GetPendingFlows(cntx context.Context, w http.ResponseWriter, r *http.Request) {
	logger.Info(ctx, "Received Get Pending Flows request")
...
func (fh *FlowHandle) GetFlows(cntx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	logger.Infow(ctx, "Received Get Flows specific to flowID and deviceID", log.Fields{"flowId": flowIDStr, "DeviceID": deviceID})

	flowID, parseErr := strconv.ParseUint(flowIDStr, 10, 64)
	if parseErr != nil {
		logger.Errorw(ctx, "Hitesh Failed to parse flowIDStr from string to uint64", log.Fields{"flowIDStr": flowIDStr, "Reason": parseErr.Error()})
		w.WriteHeader(http.StatusInternalServerError)
		logger.Errorw(ctx, "chhabra Failed to parse flowIDStr from string to uint64", log.Fields{"flowIDStr": flowIDStr, "Reason": parseErr.Error()})
		return
	}
