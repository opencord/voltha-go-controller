...
func (dh *DhcpRelayHandle) GetAllocations(cntx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Errorw(ctx, "Error occurred while marshaling dhcp allocation response", log.Fields{"Error": err})
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		logger.Errorw(ctx, "error in sending dhcp allocations response", log.Fields{"Error": err})
		w.WriteHeader(http.StatusInternalServerError)
	}
