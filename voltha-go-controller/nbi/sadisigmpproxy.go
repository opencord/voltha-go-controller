...
func (iph *IgmpProxyHandle) DelIgmpProxyInfo(cntx context.Context, w http.ResponseWriter, r *http.Request) {
	// logger.Info(ctx, "Inside DelIgmpProxyInfo method")

	// vars := mux.Vars(r)
	// outgoingIgmpVlanID := vars["outgoingigmpvlanid"]

	// logger.Infow(ctx, "Inside DelIgmpProxyInfo method", log.Fields{"req": outgoingIgmpVlanID})

	// profileID := "mvlan" + outgoingIgmpVlanID
	// // HTTP response with 202 accepted for service delete request
	// w.WriteHeader(http.StatusAccepted)

	// logger.Infow(ctx, "Inside DelIgmpProxyInfo method", log.Fields{"name": profileID})
	// go app.GetApplication().DelIgmpProfile(cntx, profileID)
}
