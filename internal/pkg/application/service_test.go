...
func TestVoltApplication_DeepEqualServicecfg(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "nvs.IsOption82Disabled != evs.IsOption82Disabled",
			args: args{
				evs: &VoltServiceCfg{
					IsOption82Disabled: true,
				},
				nvs: &VoltServiceCfg{},
			},
			want: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "VoltApplication_DeepEqualServicecfg", "nvs.Name != evs.Name", "nvs.UniVlan != evs.UniVlan",
				...
				"evs.MacLearning == MacLearningNone", "nvs.PonPort != evs.PonPort",
				"nvs.IsOption82Disabled != evs.IsOption82Disabled", "nvs.IgmpEnabled != evs.IgmpEnabled",
