...
func (va *VoltApplication) UpdateVnet(vv *VoltVnet) error {
	...
	vv.WriteToDb()
	logger.Infow(ctx, "Updated VNET TO DB", log.Fields{"vv": *vv})
	return nil
}
...
func (vpv *VoltPortVnet) ProcessDhcpSuccess(res *layers.DHCPv4) {
	...
	logger.Infow(ctx, "Services Configured", log.Fields{"Services": vpv.services, "Count": vpv.servicesCount.Load()})
...
func (vpv *VoltPortVnet) PortUpInd(device *VoltDevice, port string) {
	...
	if !device.isVlanEnabled(vpv.SVlan) {
		logger.Warnw(ctx, "Ignoring Vnet Port UP indication: Vlan not UP", log.Fields{"Port": vpv.Port, "Device": device.Name, "SVlan": vpv.SVlan})
		return
	}
...
func (vpv *VoltPortVnet) SetMacAddr(addr net.HardwareAddr) {
	...
	d, err := GetApplication().GetDeviceFromPort(vpv.Port)
	if err != nil {
	...
	if vpv.FlowsApplied && d.isVlanEnabled(vpv.SVlan) {
		// no HSIA flows for multicast service
		if !vpv.McastService {
			vpv.services.Range(AddDsHsiaFlows)
		}
...
func (vpv *VoltPortVnet) AddSvc(svc *VoltService) {
	...
	//Check Vlan status before pushing flows
	if d.isVlanEnabled(vpv.SVlan) {
		...
		logger.Warnw(ctx, "Not pushing Service Flows : vlan Not Enabled", log.Fields{"Service": svc.Name, "Device": vpv.Device, "Port": vpv.Port, "Svlan": vpv.SVlan})
	}
	vpv.WriteToDb()
}
...
func ClearServiceCounters(key, value interface{}) bool {
	...
	if svc.IgmpEnabled && svc.EnableMulticastKPI {
		db.DelAllServiceChannelCounter(svc.Name)
	}
	return true
}
...
func (vpv *VoltPortVnet) ForceWriteToDb() {
	...
	if b, err := json.Marshal(vpv); err == nil {
		db.PutVpv(vpv.Port, uint16(vpv.SVlan), uint16(vpv.CVlan), uint16(vpv.UniVlan), string(b))
	}
}
...
func (vpv *VoltPortVnet) DelFromDb() {
	...
	db.DelVpv(vpv.Port, uint16(vpv.SVlan), uint16(vpv.CVlan), uint16(vpv.UniVlan))
}
...
func (va *VoltApplication) CreateVpvFromString(b []byte, hash string) {
	...
	if err := json.Unmarshal(b, &vpv); err == nil {
		...
		logger.Debugw(ctx, "Added VPV from string", log.Fields{"VPV": vpv, "Services": vpv.services})
	}
}
...
func (va *VoltApplication) RestoreVnetsFromDb() {
	...
	for _, net := range vnets {
		...
		logger.Debugw(ctx, "Retrieved VNET", log.Fields{"VNET": vnet})
		va.AddVnet(vnet.VnetConfig, &vnet.VnetOper)
...
func (va *VoltApplication) PushDevFlowForDevice(device *VoltDevice) {
	logger.Infow(ctx, "PushDevFlowForDevice", log.Fields{"Vnets": va.VnetsByName})
...
func (va *VoltApplication) DeleteDevFlowForVlan(vnet *VoltVnet) {
	logger.Infow(ctx, "DeleteDevFlowForVlan", log.Fields{"Devices": va.DevicesDisc, "SVlan": vnet.SVlan, "CVlan": vnet.CVlan})
	delflows := func(key interface{}, value interface{}) bool {
		...
		if vnetListIntf, ok := device.ConfiguredVlanForDeviceFlows.Get(VnetKey(vnet.SVlan, vnet.CVlan, 0)); ok {
			...
			if vnetList.Length() != 0 {
				...
				device.checkVlanReqAssociation(vnet.SVlan, vnet.Name, true)
				return true
			}
		}
		if portID, err := va.GetPortID(device.NniPort); err == nil {
...
func (va *VoltApplication) DeleteDevFlowForDevice(device *VoltDevice) {
	logger.Infow(ctx, "DeleteDevFlowForDevice", log.Fields{"Vnets": va.VnetsByName})
	delicmpv6 := func(key, value interface{}) bool {
		...
		if vnetListIntf, ok := device.ConfiguredVlanForDeviceFlows.Get(VnetKey(vnet.SVlan, vnet.CVlan, 0)); ok {
			...
			logger.Warnw(ctx, "ICMPv6 Flow map entry not found for Vnet", log.Fields{"Vnets": va.VnetsByName})
			return true
		}
		nniPortID, err := va.GetPortID(device.NniPort)
		if err != nil {
...
func (va *VoltApplication) DeleteDevFlowForVlanFromDevice(vnet *VoltVnet, deviceSerialNum string) {
	...
	delflows := func(key interface{}, value interface{}) bool {
		...
		if vnetListIntf, ok := device.ConfiguredVlanForDeviceFlows.Get(VnetKey(vnet.SVlan, vnet.CVlan, 0)); ok {
			...
			logger.Warnw(ctx, "Dev Flow map entry not found for Vnet", log.Fields{"Vnets": va.VnetsByName, "PodReboot": vgcRebooted, "VnetDeleteInProgress": vnet.DeleteInProgress})
			return true
		}
		if portID, err := va.GetPortID(device.NniPort); err == nil {
			if state, _ := cntlr.GetController().GetPortState(device.Name, device.NniPort); state != cntlr.PortStateUp {
...
func ProcessIcmpv6McGroup(device string, delete bool) error {
	...
	if delete {
		...
		for _, port := range receivers {
			group.Buckets = append(group.Buckets, port)
		}
	}
...
func (vpv *VoltPortVnet) FlowRemoveFailure(cookie string, device string, errorCode uint32, errReason string) {
	...
	if device, _ := GetApplication().GetDeviceFromPort(vpv.Port); device != nil {
		device.checkVlanReqAssociation(vpv.SVlan, "", false)
	}
...
func (vv *VoltVnet) FlowRemoveSuccess(cookie string, device string) {
	...
	if d := GetApplication().GetDevice(device); d != nil {
		...
		if !present && len(vv.PendingDeleteFlow[device]) == 0 {
			d.checkVlanReqAssociation(vv.SVlan, vv.Name, true)
...
func (vv *VoltVnet) FlowRemoveFailure(cookie string, device string, errorCode uint32, errReason string) {
	...
	if flowMap, ok := vv.PendingDeleteFlow[device]; ok {
		if _, ok := flowMap[cookie]; ok {
			...
			//Check and update failure for pending disable request
			if d := GetApplication().GetDevice(device); d != nil {
				d.checkVlanReqAssociation(vv.SVlan, vv.Name, false)
			}
