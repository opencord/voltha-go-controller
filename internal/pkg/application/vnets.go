...
func (va *VoltApplication) BuildDsDhcpTrapFlows(ctx context.Context, device *VoltDevice, port string) (*of.VoltFlow, error) {
...
func (va *VoltApplication) AddDhcpTrapFlows(cntx context.Context, vd *VoltDevice, p *VoltPort) error {
	if vd.GlobalDhcpFlowAdded {
		logger.Info(ctx, "Global Dhcp flow already exists")
		return nil
	}

	flows, err := va.BuildDsDhcpTrapFlows(cntx, vd, p.Name)
	if err == nil {
