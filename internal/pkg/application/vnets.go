...
func (vpv *VoltPortVnet) setDevice(cntx context.Context, device string) {
	...
	vpv.RangeOnServices(cntx, vpv.setDeviceInService)
}
...
func (vpv *VoltPortVnet) PortUpInd(cntx context.Context, device *VoltDevice, port string) {
	...
	vpv.setDevice(cntx, device.Name)
...
// setDeviceInService to update device in service
func (vpv *VoltPortVnet) setDeviceInService(cntx context.Context, key, value interface{}) bool {
	svc := value.(*VoltService)
	logger.Infow(ctx, "Updating Device ID for service", log.Fields{"ServiceName": svc.Name})
	svc.Device = vpv.Device
	vpv.services.Store(svc.Name, svc)
	svc.WriteToDb(cntx)
