...
// as the identity. The SB is abstracted by OFAgent and the OFAgent transacts
...
func (d *VoltDevice) pushFlowsForUnis() {
	...
	logger.Infow(ctx, "NNI Discovered. Sending Port UP Ind for UNIs", log.Fields{"Uni Ports": d.Ports})
...
type VoltApplication struct {
	...
	portAlarmProfileCacheLock sync.Mutex
	vendorID                  string
}
...
func (d *VoltDevice) GetFlowEventRegister(flowModType of.Command) *util.ConcurrentMap {

	switch flowModType {
	case of.CommandDel:
		return d.FlowDelEventMap
	case of.CommandAdd:
		return d.FlowAddEventMap
	default:
		logger.Error(ctx, "Unknown Flow Mod received")
	}
	return util.NewConcurrentMap()
}
...
func (d *VoltDevice) UnRegisterFlowEvent(cookie string, flowModType of.Command) {
	...
	flowEventMap := d.GetFlowEventRegister(flowModType)
...
func (va *VoltApplication) DelDevice(device string) {
	...
	if vdIntf, ok := va.DevicesDisc.Load(device); ok {
		...
		db.DelAllRoutesForDevice(device)
		va.HandleFlowClearFlag(device, vd.SerialNum, vd.SouthBoundID)
		db.DelAllGroup(device)
		db.DelAllMeter(device)
		db.DelAllPorts(device)
...
// PortAddInd : This is a PORT add indication coming from the OFAgent, which is essentially
...
func (va *VoltApplication) ProcessIgmpDSFlowForMvlan(d *VoltDevice, mvp *MvlanProfile, addFlow bool) {
	...
	if addFlow {
		if d.isVlanEnabled(mvp.Mvlan) && portState {
...
// // ProcessDevFlowForDevices - Process DS ICMPv6 & ARP flow for provided device and vnet profile
// // device - Device Obj
// // vnetName - vnet profile name
// // enabled - vlan enabled/disabled - based on the status, the flow shall be added/removed
// func (va *VoltApplication) ProcessDevFlowForDevices(device *VoltDevice, vnetName string, enabled bool) {
// 	logger.Infow(ctx, "Processing Vlan Status Update for ICMPv6", log.Fields{"Vnet": vnetName, "Status": enabled})

// 	if vnetIntf, ok := va.VnetsByName.Load(vnetName); ok {
...
func (va *VoltApplication) HandleFlowClearFlag(deviceID string, serialNum, southBoundID string) {
	...
	if ok && dev != nil {
		...
		ProcessIcmpv6McGroup(deviceID, true)
	} else {
...
func (va *VoltApplication) UpdateDeviceSerialNumberList(oldOltSlNo string, newOltSlNo string) {
	...
	if voltDevice != nil {
		...
		sendUpdateSerialNumberResp(oldOltSlNo, newOltSlNo, "FAILURE")
		...
		sendUpdateSerialNumberResp(oldOltSlNo, newOltSlNo, "SUCCESS")

	}
}

func sendUpdateSerialNumberResp(oldOltSlNo string, newOltSlNo string, status string) {
	return
}
