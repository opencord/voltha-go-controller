...
func (d *VoltDevice) pushFlowsForUnis(cntx context.Context, nniPort string) {
	...
	d.Ports.Range(func(key, value interface{}) bool {
		...
		for _, vpv := range vnets.([]*VoltPortVnet) {
			vpv.VpvLock.Lock()
			if ok, activeNni := vpv.IsServiceActivated(cntx); ok && (nniPort == activeNni) {
				logger.Debugw(cntx, "sending PortUpInd for Unis after nni discovery", log.Fields{"activeNni": activeNni, "uniport": port})
				vpv.PortUpInd(cntx, d, port, activeNni, false)
			}
			vpv.VpvLock.Unlock()
		}
		return true
