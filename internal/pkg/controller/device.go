...
func (d *Device) AddFlowToDb(flow *of.VoltSubFlow) {
	if b, err := json.Marshal(flow); err == nil {
		db.PutFlow(d.ID, flow.Cookie, string(b))
	}
}
...
// getDeviceGroup to get device group
func (d *Device) getDeviceGroup(groupID uint32) (*of.Group, bool) {

	var group *of.Group
	groupInf, ok := d.groups.Load(groupID)
	if ok {
		group = groupInf.(*of.Group)
	}
	return group, ok
}
...
func (d *Device) AddGroupToDb(group *of.Group) {
	if b, err := json.Marshal(group); err == nil {
		...
		db.PutGroup(d.ID, group.GroupID, string(b))
	}
}
...
func (d *Device) WritePortToDb(port *DevicePort) {
	...
	if b, err := json.Marshal(port); err == nil {
		db.PutPort(d.ID, port.ID, string(b))
	}
}
...
func (d *Device) UpdateFlows(flow *of.VoltFlow, devPort *DevicePort) {
	...
	flowQueue := d.getFlowQueueForUniID(uint32(devPort.ID))
	if flowQueue != nil {
...
func (d *Device) getFlowQueueForUniID(id uint32) *UniIDFlowQueue {
