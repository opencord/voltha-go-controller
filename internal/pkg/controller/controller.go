...
type VoltController struct {
	...
	ofagentEndPoint         string
	vagent                  map[string]*ofagent.OFAgent
	ctx                     context.Context
	app                     intf.App
	RebootFlow              bool
...
func NewController(ctx context.Context, app intf.App) intf.IOFClientAgent {
...
func (v *VoltController) AddDevice(config *intf.OFClientCfg) intf.IOFClient {
...
func (v *VoltController) AddNewDevice(config *intf.OFClientCfg) {
...
func (v *VoltController) DeviceRebootInd(dID string, srNo string, sbID string) {
	...
	db.DelAllRoutesForDevice(dID)
	db.DelAllGroup(dID)
	db.DelAllMeter(dID)
	db.DelAllPONCounters(dID)
}
...
//TriggerPendingVlanDisableReq - trigger pending vlan disable requests
func (v *VoltController) TriggerPendingVlanDisableReq(device string) {
	v.app.TriggerPendingVlanDisableReq(device)
}
...
// AddOFAgent to add the ofagent
func (v *VoltController) AddOFAgent(vep string, ofa *ofagent.OFAgent) {
	v.vagent[vep] = ofa
}

// OFAgent to get ofagent info
func (v *VoltController) OFAgent(vep string) (*ofagent.OFAgent, error) {
	ofa, ok := v.vagent[vep]
	if ok {
		return ofa, nil
	}
	return nil, errors.New("OFA Not Registered")
}
...
func (v *VoltController) AddFlows(port string, device string, flow *of.VoltFlow) error {
	...
	if isMigrationRequired {
		...
		for _, subFlow := range flow.SubFlows {
			...
			d.DelFlowWithOldCookie(subFlow)
			d.AddFlow(subFlow)
		}
	} else {
