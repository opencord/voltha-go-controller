...
func (ppt *PendingProfilesTask) Start(ctx context.Context, taskID uint8) error {
	...
	//Trigger Pending Vlan Disable Tasks
	logger.Warnw(ctx, "Pending Vlan Disable Task Triggered", log.Fields{"Device": ppt.device.ID})
	GetController().TriggerPendingVlanDisableReq(ppt.device.ID)
	logger.Warnw(ctx, "Pending Vlan Disable Task Completed", log.Fields{"Device": ppt.device.ID})
