...
func (db *Database) Put(fullKeyPath, value string) error {
	return database.kvc.Put(context.Background(), fullKeyPath, value)
}

// Get to retrieve value from database
func (db *Database) Get(key string) (string, error) {
	kv, err := database.kvc.Get(context.Background(), key)
	if err != nil {
		return "", err
	}
	if kv != nil {
...
func (db *Database) Del(fullPath string) {
	if err := database.kvc.Delete(context.Background(), fullPath); err != nil {
		logger.Infof(ctx, "The path doesn't exist", fullPath)
	}
}

// DeleteAll to delete all value from database
func (db *Database) DeleteAll(fullPath string) error {
	if err := database.kvc.DeleteWithPrefix(context.Background(), fullPath); err != nil {
		logger.Infof(ctx, "The key doesn't exist", fullPath)
		return err
	}
	return nil
}
...
func (db *Database) DeleteAllUnderHashKey(hashKeyPrefix string) error {
	if err := database.kvc.Delete(context.Background(), hashKeyPrefix); err != nil {
		logger.Infof(ctx, "The key path doesn't exist", hashKeyPrefix)
		return err
	}
	return nil
}
...
func (db *Database) List(key string) (map[string]*kvstore.KVPair, error) {
	kv, err := database.kvc.List(context.Background(), key)
	if err != nil {
		return nil, err
	}
	if kv != nil {
...
func (db *Database) PutOlt(deviceID string, value string) error {
	key := fmt.Sprintf(GetKeyPath(DevicePath), deviceID)
	return database.kvc.Put(context.Background(), key, value)
}

// DelOlt to delete olt info
func (db *Database) DelOlt(deviceID string) {
	key := fmt.Sprintf(GetKeyPath(DevicePath), deviceID)
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
}
...
func (db *Database) DelFlow(deviceID string, flowID uint64) {
	key := fmt.Sprintf(GetKeyPath(DeviceFlowPath), deviceID) + strconv.FormatUint(flowID, 10)
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelGroup(deviceID string, groupID uint32) {
	key := fmt.Sprintf(GetKeyPath(DeviceGroupPath), deviceID) + strconv.FormatUint(uint64(groupID), 10)
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelPort(deviceID string, portID uint32) {
	key := fmt.Sprintf(GetKeyPath(DevicePortPath), deviceID) + strconv.FormatUint(uint64(portID), 10)
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelDeviceMeter(deviceID string, meterID uint32) {
	key := fmt.Sprintf(GetKeyPath(DeviceMeterPath), deviceID) + strconv.FormatUint(uint64(meterID), 10)
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelService(name string) {
	key := GetKeyPath(ServicePath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelVnet(name string) {
	key := GetKeyPath(VnetPath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelVpv(port string, SVlan uint16, CVlan uint16, UniVlan uint16) {
	name := port + fmt.Sprintf("-%v-%v-%v", SVlan, CVlan, UniVlan)
	key := GetKeyPath(VpvPath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
...
func (db *Database) DelMvlan(mvlan uint16) {
	name := strconv.FormatInt(int64(mvlan), 10)
	key := GetKeyPath(MvlanPath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
...
func (db *Database) DelIgmpProfile(name string) {
	key := GetKeyPath(IgmpProfPath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelMcastConfig(name string) {
	key := GetKeyPath(McastConfigPath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelMeter(name string) {
	key := GetKeyPath(MeterPath) + name
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelAllMeter(device string) {
...
func (db *Database) DelIgmpGroup(id string) {
	key := GetKeyPath(IgmpGroupPath) + id
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Warnf(ctx, "The key doesn't exist", key)
	}
}
...
func (db *Database) DelIgmpDevice(mvlan of.VlanType, gid string, gip net.IP, device string) {
	key := GetKeyPath(IgmpDevicePath) + mvlan.String() + "/" + gid + "/" + gip.String() + "/" + device
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Warnf(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelIgmpChannel(mvlan of.VlanType, gName string, device string, gip net.IP) {
	key := GetKeyPath(IgmpChannelPath) + mvlan.String() + "/" + gName + "/" + device + "/" + gip.String()
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Warnf(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelIgmpRcvr(mvlan of.VlanType, gip net.IP, device string, rcvr string) {
	key := GetKeyPath(IgmpPortPath) + mvlan.String() + "/" + gip.String() + "/" + device + "/" + rcvr
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Warnf(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelAllIgmpRcvr(mvlan of.VlanType, gip net.IP, device string) {
	key := GetKeyPath(IgmpPortPath) + mvlan.String() + "/" + gip.String() + "/" + device + "/"
...
func (db *Database) DelAllRoutesForDevice(device string) {
...
func (db *Database) PutNbDevicePort(device string, ponPortID uint32, value string) {
	...
	database.kvc.Put(context.Background(), key, value)
}
...
func (db *Database) DelNbDevicePort(device string, ponPortID uint32) {
	...
	database.kvc.Delete(context.Background(), key)
}
...
func (db *Database) DelMigrationInfo() {
	key := GetKeyPath(MigrationInfoPath)
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelPonCounter(device, ponID string) {
	key := GetKeyPath(PonCounterPath) + device + "/" + ponID
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelPonChannelCounter(device, ponID, channel string) {
	key := GetKeyPath(PonCounterPath) + device + "/" + ponID + "/" + ChannelCounterPath + channel
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelAllPONCounters(device string) {
	key := GetKeyPath(PonCounterPath) + device + "/"
	db.DeleteAll(key)
}
...
func (db *Database) DelServiceChannelCounter(serviceName, channel string) {
	key := GetKeyPath(ServiceCounterPath) + serviceName + "/" + ChannelCounterPath + channel
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelAllServiceChannelCounter(serviceName string) {
	key := GetKeyPath(ServiceCounterPath) + serviceName + "/" + ChannelCounterPath
	db.DeleteAllUnderHashKey(key)
}
...
func (db *Database) DelMigrateServicesReq(deviceID string, vnet string) {
	key := fmt.Sprintf(GetKeyPath(ServicesMigrateReqPath), deviceID) + vnet
	if err := database.kvc.Delete(context.Background(), key); err != nil {
		logger.Infof(ctx, "The key doesn't exist", key)
	}
...
func (db *Database) DelAllMigrateServicesReq(deviceID string) {
	key := fmt.Sprintf(GetKeyPath(ServicesMigrateReqPath), deviceID)
	if err := db.DeleteAllUnderHashKey(key); err != nil {
		logger.Warnf(ctx, "Delete All failed: The key doesn't exist", key)
	}
